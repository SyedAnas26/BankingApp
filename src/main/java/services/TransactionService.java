...
public class TransactionService {
    ...
    public Response createTransaction(Transaction newTransaction) throws Exception {
        ...
        String insertQuery = "insert into transaction(account_id,mode_of_payment,transaction_type,amount,transaction_status,to_account_id) values ('" +
                ...
                "','" + newTransaction.getAmount() + "','" + newTransaction.getTransactionStatus().getId() + "','" + newTransaction.getToAccountId() + "')";

        try {

            newTransaction.setId(DbConnector.insert(insertQuery));

            if (newTransaction.getTransactionStatus() != TransactionStatus.FAILED) {
                ...
                if (newTransaction.getToAccountId() != 0) {
                    String getQuery2 = "select * from account where id=" + newTransaction.getToAccountId();
                    float finalChangeInAmount2 = -changeInAmount;

                    DbConnector.get(getQuery2, rs -> {
                        if (rs.next()) {
                            DbConnector.update("update account set balance=" + (rs.getFloat("balance") + finalChangeInAmount2) + "where id=" + rs.getInt("id"));
                        }
                        return null;
                    });
                }
